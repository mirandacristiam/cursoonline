...
// API para gestionar perfil de administrador/estudiante
// Ahora usa procedimientos almacenados sp_admin_*
...
// sp_exec: ejecuta SP sin retornar filas (UPDATE/INSERT)
...
// sp_scalar: ejecuta SP y retorna un solo valor
...
// foto_paths: retorna [ruta_fisica, url_publica] según el rol
function foto_paths($rol) {
    if ($rol === ROL_ADMIN_TOTAL) {
        return [ADMIN_FOTO_PATH, ADMIN_FOTO_URL];
    }
    return [STUDENT_FOTO_PATH, STUDENT_FOTO_URL];
}

list($FOTO_PATH, $FOTO_URL) = foto_paths($id_rol);
...
if ($accion === 'actualizar_perfil') {
    ...
    $tipos_validos = ['CC', 'CE', 'TI', 'Pasaporte', 'NIT', 'Otro'];
    if (!empty($tipo_documento) && !in_array($tipo_documento, $tipos_validos)) {
        http_response_code(400);
        echo json_encode(['estado' => 'error', 'mensaje' => 'Tipo de documento no válido.']);
        exit();
    }

    try {
        sp_exec($pdo, 'CALL sp_admin_actualizar_perfil(
...
elseif ($accion === 'subir_foto') {
    ...
    $ruta_destino = $FOTO_PATH . $nombre_unico;
    ...
    try {
        $foto_anterior = sp_scalar($pdo, 'CALL sp_admin_obtener_foto(:id)', [':id' => $id_usuario]);

        if ($foto_anterior && file_exists($FOTO_PATH . basename($foto_anterior))) {
            unlink($FOTO_PATH . basename($foto_anterior));
        }

        $ruta_relativa = $FOTO_URL . $nombre_unico;

        sp_exec($pdo, 'CALL sp_admin_subir_foto(:id, :foto)', [
            ':id'   => $id_usuario,
            ':foto' => $ruta_relativa,
        ]);
        ...
        error_log("[FOTO ERROR] " . $e->getMessage());
        http_response_code(500);
...
elseif ($accion === 'eliminar_foto') {
    ...
    try {
        ...
        sp_exec($pdo, 'CALL sp_admin_eliminar_foto(:id)', [':id' => $id_usuario]);

        echo json_encode(['estado' => 'ok', 'mensaje' => 'Foto de perfil eliminada.']);

    } catch (PDOException $e) {
        error_log("[FOTO DELETE ERROR] " . $e->getMessage());
        http_response_code(500);
...
elseif ($accion === 'cambiar_password') {
    ...
    try {
        $hash = sp_scalar($pdo, 'CALL sp_admin_obtener_hash(:id)', [':id' => $id_usuario]);
        ...
        sp_exec($pdo, 'CALL sp_admin_cambiar_password(:id, :hash)', [
            ':id'   => $id_usuario,
            ':hash' => $nuevo_hash,
        ]);
    ...
    error_log("[PERFIL FATAL] " . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
    http_response_code(500);
